...
def create_canvas_and_draw_circles(output_folder, radius,file_i):
    # 創建一個黑色畫布
    canvas_width = 1442
    canvas_height = 1080
    canvas = np.zeros((canvas_height, canvas_width, 3), dtype=np.uint8)
    output_txt_path='D:\\PUPU\\odepcellarray\\虛擬操作\\virtual_particle_points(對應)'
    # 隨機生成 20 個點座標
    points = []
    for _ in range(20):
    ...
    # 將座標寫入文本檔案
    output_txt_path = os.path.join(output_txt_path, f'{file_i}.txt')
...
for i in range (filenumber):
    create_canvas_and_draw_circles(output_folder, radius,file_i)
    file_i+=1